...
def objective(trial,dirname):
    ...
    metrics_summary = df[["max_fidelity", "ttime", "cpu_time", "generations","max_fitness"]].astype(float).agg(["mean", "std", "max"])
    ...
    return mean_fitness*max_fidelity
...
if __name__ == "__main__":
    ...
    dirname = f"optuna_trials_{timestamp}"

    if not os.path.exists(dirname):
        os.makedirs(dirname)

    study = optuna.create_study(direction="maximize")
    study.optimize(lambda trial: objective(trial, dirname), n_trials=10)
